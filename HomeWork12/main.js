...
buttonGetPersons.addEventListener('click', async function () {
    ...
    let filmIndex = document.getElementById('film').value;
    if (filmIndex == '') {
        filmIndex = 2
    }
...
nextBtn.addEventListener('click', async function () {
    if (curentPage == 1) {
        ...
        prevBtn.classList.add('visible-btn');
    }
    curentPage += 1;
    continerDiv.innerHTML = ' Loading... ';
    await GetPlanets(curentPage);
...
prevBtn.addEventListener('click', async function () {
    ...
    if (curentPage == 1) {
        prevBtn.classList.add('hidden-btn');
        prevBtn.classList.remove('visible-btn');
    }
    continerDiv.innerHTML = ' Loading... ';
    await GetPlanets(curentPage);
});
...
async function GetPlanets(page) {
    ...
    continerDiv.innerHTML = '';
    if (curentPage == Math.ceil(planetsOfFilms.data.count / numberPlanetsOnPage)) {
        nextBtn.classList.add('hidden-btn');
        nextBtn.classList.remove('visible-btn');
    } else if (curentPage == Math.ceil(planetsOfFilms.data.count / numberPlanetsOnPage) - 1) {
        nextBtn.classList.remove('hidden-btn');
        nextBtn.classList.add('visible-btn');
    }
    planetsOfFilms.data.results.forEach(planet => {
        continerDiv.innerHTML += `
        <div class='planet element'><h3>Planet name: ${planet.name}</h3></div>
        `
    });
}
